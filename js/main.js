...
function wordsList(str, subStr) {
    ...
    let set = new Set();
    arr.forEach(elem => {
        if(elem.includes(subStr)) set.add(elem);
    });
    return set;
}
...
function getLocalDate(date, isSeconds, isISO) {
    ...
    // let local = `${d}.${m}.${y}, ${h}:${min}:${sec}`;
    let localNoSec = `${d}.${m}.${y}, ${h}:${min}`;
    let iso = `${y}-${m}-${d}, ${h}:${min}:${sec}`;
    let isoNoSec = `${y}-${m}-${d}, ${h}:${min}`;

    if (isISO && isSeconds) return iso;
    else if (isISO && !isSeconds) return isoNoSec;
    // else if (!isISO && isSeconds) return local;
    else if (!isISO && !isSeconds) return localNoSec;
    else if (!isISO && isSeconds) return date.toLocaleString();
}
...
function getDateAgo(date, days) {
        let myDate = new Date(date);
        let msAgo = days * 1000 * 3600 * 24;
        let ms = myDate.getTime();
        let res = new Date(ms - msAgo);
...
Car.prototype = {
    ...
    set used (value) {
        if (value === 'new' && this.year !== new Date().getFullYear()) {
